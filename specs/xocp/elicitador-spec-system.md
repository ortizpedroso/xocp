...
**Fora de escopo pra pesquisa do Elicitador:** comparação de mercado, preço de concorrente, ou qualquer pesquisa que responda "vale a pena construir isso". A pesquisa que o Elicitador faz (seção 3.3) é **técnica** — stack, versão, framework, norma — nunca de viabilidade de negócio.

### 2.5 A Spec só existe se estiver salva em arquivo — nunca só na conversa

**Regra dura, achada em uso real — a parte mais importante de todo o processo de elicitação.**

Uma Spec "apresentada" na conversa, mas nunca gravada em arquivo, **não existe**. Se a sessão terminar, se o contexto for compactado, se alguém perguntar "como está o projeto segundo a spec" numa sessão nova — sem arquivo, a resposta correta seria "não sei", porque de fato não existe nada persistido.

**Comportamento obrigatório, verificável:**

1. **Escreva o arquivo cedo, não só no final.** Assim que houver informação suficiente pra um primeiro rascunho, grave em `specs/<slug>.md` — não espere ter 100% das respostas.
2. **Depois de aprovação humana**, atualize o arquivo pra refletir a versão aprovada.
3. **Nunca considere a elicitação concluída sem confirmar que o arquivo existe de verdade** — releia do disco antes de dizer "spec pronta".
4. Se alguém perguntar sobre a spec e o arquivo não existir: procure ativamente (`specs/`, nomes alternativos) antes de concluir que não existe.
